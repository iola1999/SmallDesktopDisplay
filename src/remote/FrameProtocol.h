...
// 查表 CRC 的表放 PROGMEM（ESP8266 上省 1KB RAM）；host 测试环境没有 PROGMEM，
// 用普通数组走同一套代码路径。
#if defined(ESP8266) || defined(ARDUINO_ARCH_ESP8266)
#include <pgmspace.h>
#define SDD_CRC32_TABLE_ATTR PROGMEM
#define SDD_CRC32_ENTRY(table, index) pgm_read_dword(&(table)[index])
#else
#define SDD_CRC32_TABLE_ATTR
#define SDD_CRC32_ENTRY(table, index) (table)[index]
#endif
...
// 反射多项式 0xEDB88320 的标准 256 项查表（与 zlib crc32 相同）。
// 函数内 static 保证头文件被多个编译单元包含时只有一份表。
inline const uint32_t *crc32Table()
{
  static const uint32_t kTable[256] SDD_CRC32_TABLE_ATTR = {
...
// 查表实现：每字节 1 次查表 + 移位，替代原来的每字节 8 次位循环。
// 全屏帧 (~115KB) 的 CRC 时间从数十毫秒降到个位数毫秒，直接缩短帧上屏耗时。
inline uint32_t crc32Update(uint32_t crc, const uint8_t *data, std::size_t length)
{
  const uint32_t *table = crc32Table();
  for (std::size_t index = 0; index < length; ++index)
  {
    crc = (crc >> 8) ^ SDD_CRC32_ENTRY(table, (crc ^ data[index]) & 0xFFU);
  }
  return crc;
}
