...
describe("Conway life view model", () => {
  test("generates deterministic bounded seeds", () => {
    const first = buildConwayLifeViewModel({seed: "slot-1", generation: 0, columns: 8, rows: 6, cellSize: 6});
    const again = buildConwayLifeViewModel({seed: "slot-1", generation: 0, columns: 8, rows: 6, cellSize: 6});
    const other = buildConwayLifeViewModel({seed: "slot-2", generation: 0, columns: 8, rows: 6, cellSize: 6});
