...
function RowsDetailPage({model}: {model: RowsDetailViewModel}) {
  return (
    <Screen fontKey={model.fontKey} backgroundColor="#070a10">
      <Box style={{x: 8, y: 8, width: 224, height: 224, borderRadius: 14, borderColor: "#2c3644", borderWidth: 2}} />
      <Text style={{x: 20, y: 18, width: 190, height: 28, fontSize: 22, color: "#edf2f8"}}>{model.title}</Text>
      <Text style={{x: 20, y: 49, width: 190, height: 18, fontSize: 13, color: "#6f87ab"}}>{model.subtitle}</Text>
...
function Row({row, index}: {row: DetailRowViewModel; index: number}) {
  return (
    <>
      ...
      <Text style={{x: 92, y: 82 + index * 28, width: 128, height: 20, fontSize: 16, color: "#e2ebf5"}}>{row.value}</Text>
    </>
  );
}
